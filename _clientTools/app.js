function App(args){
  console.log("given args",args);
   ...
   let props  = {...defaultArgs,...args}
    ...
    self.loadProcedure = (arguments)=>{
        console.log(`loading app with args:`,arguments);
    ...
    function loadBtns()
    {
      let dataRecords = document.querySelectorAll(`.record-tr`);
    ...
    {
      let wrap = {payLoad:record,sourceContext:props.sourceContext};
    ...
    self.load = (args)=>{
      ...
      const { createApp,ref } = Vue
        ...
        fetch(props.urlSet.url).then(r=>r.json()).then(wrapped=>wrapped.subject).then(data=>{
          makeMap(data);
